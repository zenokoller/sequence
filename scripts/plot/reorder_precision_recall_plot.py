...
NAMES = ['symbol_bits', 'loss_precision', 'loss_recall', 'reorder_precision', 'reorder_recall']


def plot(csv_path: str, title: str):
    df = pd.read_csv(csv_path, names=NAMES, index_col=0)
    agg_df = df.groupby('symbol_bits').mean()

    fig, ax = plt.subplots()
    agg_df.plot('loss_precision', 'loss_recall', marker='o', label='Loss', ax=ax)
    agg_df.plot('reorder_precision', 'reorder_recall', marker='o', label='Reordering', ax=ax)
