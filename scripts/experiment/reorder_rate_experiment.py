...
from scripts.experiment.data_utils import compute_loss_rate, compute_reordering_rate
...
SYMBOL_BITS = 2
MAX_REORDER_DIST = 500
TITLE = f'Reordering Configurations ({SYMBOL_BITS} symbol bits; ' \
        f'max_reorder_dist: {MAX_REORDER_DIST})'
# TITLE = f'Reordering and Loss Configurations ({SYMBOL_BITS} symbol bits; ' \
#         f'max_reorder_dist: {MAX_REORDER_DIST})'

EXPERIMENT_CONF = 'config/reorder_rate_2.yml'
# EXPERIMENT_CONF = 'config/reorder_rate_8.yml'
...
NETEM_CONFS = [
    ...
    # 'conf/reorder_rate/reorder_0125_loss_05_delay_10.conf',
    # 'conf/reorder_rate/reorder_025_loss_05_delay_10.conf',
    # 'conf/reorder_rate/reorder_05_loss_05_delay_10.conf',
    # 'conf/reorder_rate/reorder_1_loss_05_delay_10.conf',
]
INTERVAL = 15.  # seconds
...
def reordering_rate_to_csv(start_time: int, end_time: int, csv_path: str, _: dict):
    ...
    joined_df['loss_sequence'] = compute_loss_rate(joined_df, 'losses_sequence', 'packets')
    joined_df['loss_netem'] = compute_loss_rate(joined_df, 'losses_netem', 'packets')
    joined_df['reordering_sequence'] = compute_reordering_rate(joined_df, 'reorders_sequence',
                                                               'packets')
    joined_df['reordering_netem'] = compute_reordering_rate(joined_df, 'reorders_netem', 'packets')
